...
result_file="nfp_1PF2VF.log"
tc_result_file="tc_nfp_1PF2VF.log"
...
function add_ovs_flows() {
	...
	ovs-ofctl add-flow ovsbr0 in_port=3,actions=output:1
...
function add_tc_filter() {
        ...
        tc qdisc add dev $vfr1 ingress
        tc filter add dev $vfr1 protocol all parent ffff: flower $filter_policy src_mac 04:f4:bc:6a:8e:c0 action mirred egress redirect dev $nic
        tc filter show dev $vfr1 ingress
}
...
VMSH_PROMPT1="testpmd>" VMSH_NORESULT=1 VMSH_NOLOGOUT=1 vmsh run_cmd master "/root/testpmd.sh -n 2 -c 3 -q 1 -m 1024 -w 0000:00:03.0 -w 0000:00:08.0 -f io"

#WORKING COMMAND-
#17.11 command
#testpmd -l 0,1,2 -n4 --socket-mem 1024 -w 0000:00:03.0 -w 0000:00:08.0 -- --burst=64 -i --txqflags=0xf00 --rxd=2048 --txd=2048 --nb-cores=2 --rxq=1 --txq=1 --disable-rss --forward-mode=io --auto-start --port-topology=chained

#18.11 command
#testpmd -l 0,1,2 -n 4 -w 0000:00:03.0 -w 0000:00:08.0  -- --burst=64 -i --rxd=2048 --txd=2048 --nb-cores=2 --rxq=1 --txq=1 --disable-rss --forward-mode=io --auto-start
